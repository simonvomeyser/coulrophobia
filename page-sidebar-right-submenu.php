    ...
    <?php foreach ($pages as $key => $singlePage): ?>
    	...
    	<div class="sidbar-nav-box"	<?php if(get_post()->ID===$singlePage->ID)echo 'id="sidbar-nav-box-current"';?>>
    		<a href="<?php echo get_permalink($singlePage->ID); ?>">
                <?php if (get_post_thumbnail_id( $singlePage->ID )): ?>
    			 <img alt="background image of the navigation" class="nav-box-bg-img" src="<?php echo wp_get_attachment_url(get_post_thumbnail_id( $singlePage->ID ))?>">   
                <?php endif ?>
